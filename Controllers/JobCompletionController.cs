...
{
    [Authorize]
    public class JobCompletionController : Controller
    {
        ...
        {
            JobCompletion job = new JobCompletion();

            if (id.HasValue)
            {
                var mo = _context.JobCompletions.Find(id.Value);
                if (mo != null)
                {
                    job = mo;
        ...
        [HttpPost]
        public ActionResult index(JobCompletion model, HttpPostedFileBase beforPhoto, HttpPostedFileBase afterPhoto
          )
        {
            var manger = new UserManager<ApplicationUser>(new UserStore<ApplicationUser>(new ApplicationDbContext()));
            var CurrentUser = manger.FindById(User.Identity.GetUserId());
            ...
            if (model.Id > 0)
            {
                if (model.type == "Copy")
                {
                    _context.JobCompletions.Add(model);
        ...
        {
            string _file = "";
            var file = _context.JobCompletions.Find(id.Value);
            if (file != null)
            {
                _file = file.FileName;
            }
