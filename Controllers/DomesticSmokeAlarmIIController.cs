...
{
    [Authorize]
    public class DomesticSmokeAlarmIIController : Controller
    {
        // GET: DomesticSmokeAlarmII
        public TempletDbContext _context { get; set; }
        public
            DomesticSmokeAlarmIIController()
        {
        ...
        {
            DomesticSmokeAlarmII domesticSmokeAlarm = new DomesticSmokeAlarmII();
            if (id.HasValue)
            {
                var mo = _context.DomesticSmokeAlarmIIs.Find(id.Value);
                if (mo != null)
                {
        ...
        [HttpPost]
        public ActionResult index(DomesticSmokeAlarmII model
            )
        {
            ...
            if (model.Id > 0)
            {
                if (model.type == "Copy")
                {
                    _context.DomesticSmokeAlarmIIs.Add(model);
        ...
        {
            string _file = "";
            var file = _context.DomesticSmokeAlarmIIs.Find(id.Value);
            if (file != null)
            {
                _file = file.FileName;
            }
